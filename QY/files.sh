...
do
  ...
  head -1 $line.sam | grep "^@" > header
#the first line must start with @ (header mandatory for the SAM file)
  if ! [ -s header ];then
	echo "Error: Check that the input file has SAM format with header."
	rm referencelength referencename names header
	exit 1
  fi
  rm header
...
#intermediate files are deleted
  ...
  awk '{gsub("[0-9]+[a-zA-Z]+","n",$2)}1' mispos > mi 2>/dev/null
  rm mispos
  awk '{gsub("\^[acgtnACGTN]","",$2)}1' mi > mis 2>/dev/null
  rm mi
  awk '{gsub("[^acgtnACGTN]","",$2)}1' mis > $line.mismapos 2>/dev/null
  rm mis
